...
const ManageCategoriesModal: React.FC<ManageCategoriesModalProps> = ({
  ...
  const togglePicker = (category: string) => {
    handleCancelEditing(); // Cancel name editing
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-center items-center z-50 transition-opacity duration-300">
      <div className="bg-white/80 backdrop-blur-lg border border-gray-200/50 rounded-2xl shadow-2xl p-8 w-full max-w-lg m-4 transform transition-all duration-300 scale-100">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">管理分类</h2>
              ...
              <div className="flex items-center flex-grow min-w-0">
                  <div className="relative mr-3 flex-shrink-0">
                    ...
                    {activePicker === category && (
                      <div ref={pickerRef} className="absolute top-full mt-2 z-20 w-48 p-2 bg-white rounded-lg shadow-xl border border-gray-200">
                        <div className="grid grid-cols-5 gap-2">
                          {EMOJIS.map(emoji => (
                            <button
                              key={emoji}
                              onClick={() => handleIconSelect(category, emoji)}
